...
class Content:
    #Content class for printing events.
...
def get_events_from_wrap(cal, wrap_div, base_year):
    #Extract events from a month wrap div.
    month_text = wrap_div.select_one('div.month').select_one('span.month-text').get_text()
...
def main():
    #Main function to fetch, parse, and save SNU calendar.
    ...
    year_elem = soup.select_one('div.this-year.p')
    if year_elem:
        thisyear_text = year_elem.get_text(strip=True)
        thisyear = int(''.join(map(str, extract_digits(thisyear_text))))
    else:
        # Fallback: use current year or a constant
        thisyear = datetime.now().year  # or 2026, etc.
        print("Warning: 'div.this-year.p' element not found; falling back to", thisyear)
